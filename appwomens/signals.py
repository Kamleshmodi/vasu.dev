...
from .models import ProductVariation, Clothing, Footwear, Dresses, Accessories, Bags, BeautyProducts
...
def auto_sync_category(sender, instance, **kwargs):
    ...
    if type_value == 'clothing':
        ...
        Accessories.objects.get_or_create(product=product, size=instance.size, color=instance.color)
    elif type_value == 'bags':
        Bags.objects.get_or_create(product=product, size=instance.size, color=instance.color)
    elif type_value == 'beauty':
        BeautyProducts.objects.get_or_create(product=product, size=instance.size, color=instance.color)
...
def auto_delete_synced_category(sender, instance, **kwargs):
    ...
    if type_value == 'clothing':
        ...
        Accessories.objects.filter(product=product, size=instance.size, color=instance.color).delete()
    elif type_value == 'bags':
        Bags.objects.filter(product=product, size=instance.size, color=instance.color).delete()
    elif type_value == 'beauty':
        BeautyProducts.objects.filter(product=product, size=instance.size, color=instance.color).delete()
