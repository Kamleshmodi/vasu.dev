...
COLORS = [
    ...
    ('Orange', 'Orange'), ('Cyan', 'Cyan'), ('Magenta', 'Magenta'), ('Blank', 'Blank'),
]
...
class NewProductChoices(models.TextChoices):
    ...
    SALE = 'Sale', 'Sale'
    SOAP = 'Soap', 'Soap'
    KENDALLS = 'Kendalls', 'Kendalls'

class NewProduct(models.Model):
    product_name = models.CharField(max_length=200, unique=True)
    brand = models.CharField(max_length=200)
    slug = models.SlugField(max_length=200, unique=True)
    vendor = models.ForeignKey(settings.AUTH_USER_MODEL, related_name='womens_products_created', on_delete=models.SET_NULL, null=True, blank=True)
    # === YAHAN BADLAV KIYA GAYA HAI ===
    gender = models.CharField(max_length=10, default='Women') 
    # ==================================
    designer = models.ForeignKey(Designer, on_delete=models.CASCADE, related_name='womens_products')
    price = models.DecimalField(max_digits=10, decimal_places=2)
    front_image = models.ImageField(upload_to='photos/products')
    back_image = models.ImageField(upload_to='photos/products', blank=True, null=True)
    is_available = models.BooleanField(default=True)
    category = models.ForeignKey(Category, on_delete=models.CASCADE, related_name="womens_products")
...
class SaleItems(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.sale_price = float(discounted_price)
        super().save(*args, **kwargs)

class Kendalls_editions(models.Model):
    edition_number = models.CharField(max_length=10, blank=True)
    edition_name = models.CharField(max_length=200, blank=True)
    edition_description = models.TextField(max_length=500, blank=True)
    edition_image = models.ImageField(upload_to='photos/editions')

    def __str__(self):
        return f"{self.edition_number} - {self.edition_name}"
...
class Shops(models.Model):
    ...
    is_vasu_soap = models.BooleanField(default=False)
    ...
    def front_image(self):
        return self.product.front_image

    @property
    def back_image(self):
        return self.product.back_image
